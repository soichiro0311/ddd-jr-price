...
describe("往復割引", () => {
    describe("往復の距離に応じて割引が適用されること", () => {
        it("東京から姫路の往復運賃の場合、往復割引が適用されること", () => {
            ...
            const discount = new RoundTripDiscount(travelSection);
            const basicFare = new BasicFare(travelSection, AdultChildCategory.Adult, RoundTripType.RoundTrip, 1, new Date("2024-5-11"), new Date("2024-5-13"));
    ...
    describe("往復の運賃にのみ割引が適用されること", () => {
        it("東京から姫路の片道運賃の場合、往復割引が適用されないこと", () => {
            const travelSection = new TravelSection(Station.Tokyo, Station.Himeji)
            const discount = new RoundTripDiscount(travelSection);
            const basicFare = new BasicFare(travelSection, AdultChildCategory.Adult, RoundTripType.OneWay, 1, new Date("2024-5-11"), new Date("2024-5-13"));
            const isApplyable = discount.isApplyable(basicFare)
            expect(isApplyable).toBeFalsy();
        })
    })
